...
/*   Updated: 2016/05/23 18:40:09 by dboudy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		smash_cone(t_obj *cone, t_ray *aray)
{
	...
	tmp[0] = (cone->size / cone->h) * (cone->size / cone->h);
	tmp[1] = vector_dot2dxz(&cone->dir, &cone->dir);
	tmp[2] = vector_dot2dxz(&cone->dir, &aray->dir);
	tmp[3] = vector_dot2dxz(&cone->dir, &v);
	abc[0] = vector_dotxz_subycoef(&aray->dir, &aray->dir, tmp[0]) - ((tmp[2] * tmp[2]) / tmp[1]);
	abc[1] = 2 * vector_dotxz_subycoef(&aray->dir, &v, tmp[0]) - (2 * tmp[2] * tmp[3] / tmp[1]);
	abc[2] = vector_dotxz_subycoef(&v, &v, tmp[0]) - ((tmp[3] * tmp[3]) / tmp[1]);
	...
	set_to(&v, &aray->origin);
	vector_sub(&v, &cone->origin);
	tmp[0] = vector_dot(&cone->dir, &cone->dir);
	tmp[1] = vector_dot(&cone->dir, &aray->dir);
	tmp[2] = vector_dot(&v, &v);
	tmp[3] = vector_dot(&cone->dir, &v);
	abc[0] = vector_dot(&aray->dir, &aray->dir) - ((tmp[1] * tmp[1]) / tmp[0]);
	abc[1] = 2 * vector_dot(&aray->dir, &v) - (2 * tmp[1] * tmp[3] / tmp[0]);
	abc[2] = tmp[2] - (cone->size * cone->size) - ((tmp[3] * tmp[3]) / tmp[0]);
	res[0] = abc[1] * abc[1] - 4 * abc[0] * abc[2];
	if (res[0] >= 0)
	{
		res[1] = (-abc[1] + sqrtf(res[0])) / (2 * abc[0]);
